    ...
    for (var i = this.state.storage.length - 1; i >= 0; i--) {
      for (var j = 0; j < activity?.length; j++) {
        if (this.state.storage[i].id_actividad == activity[j].id_actividad) {
          ...
          const obj = JSON.parse(this.state.storage[i].answers);
          console.log('obj ',obj);
          console.log('h ',activity[j].questions);
          if (obj) {

            const valuesArray = Object.values(obj);
            for (let i = 0; i < valuesArray.length; i++) {
              if (valuesArray[i] === activity[j].questions[i].correct) {
                notaFEvaluations = notaFEvaluations + 5;
              }    
              else {   
                notaFEvaluations = notaFEvaluations + 0;
                  }
                }
          } else {
            notaFEvaluations = notaFEvaluations + 0;
            console.log("El objeto es nulo o indefinido."); 
          }
          let cantPreguntas = activity[j].questions.length;
          notaFEvaluation = notaFEvaluations /cantPreguntas;
          console.log('noticas ',notaFEvaluations);
          console.log('cant preguntas  ',cantPreguntas);
          console.log('nota evaluacion ', notaFEvaluation);
          let progressBar = {
            check_inicio: 0,
            check_video: 0,
            check_answer: 0,
          ...
          if (this.state.storage[i].answers && this.state.storage[i].answers.length > 0) {
            progressBar = { ...progressBar, check_answers: 0.3 };
          }
